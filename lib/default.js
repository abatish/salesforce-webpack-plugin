  ...
  page: {
    ...
    template: `<apex:page
                ...
                <% if(options.remoteObject) { %>
                  <apex:remoteObjects
                    <% if(options.remoteObject && options.remoteObject.namespace) { %>
                  ...
                  >
                    <% Object.keys(options.remoteObject.models).forEach(function (modelName) { %>
                      ...
                      <% var jsShorthand = options.remoteObject.models[modelName].jsShorthand || '' %>
                      <% var finalFields = fields.map((field) => { if(field.indexOf('__c') === -1 || !packageName) { return field; } return packageName + field; }).join(',') %>
                      <apex:remoteObjectModel name="<%= packageName %><%= modelName %>" jsShorthand="<%= jsShorthand %>" fields="<%= finalFields %>">
                        <% options.remoteObject.models[modelName].objectFields.forEach(function (objField) { %>
                          <apex:remoteObjectField name="<%= objField.name %>" jsShorthand="<%= objField.jsShorthand %>" />
                        <% }) %>	
                ...
                <% } %>
                  ...
                  <script>
                    ...
                    window['<%= options.pageUrl %>'] = '{!IF($Site.BaseSecureUrl!= null, $Site.BaseSecureUrl,getURL)}/apex';
                    window['<%= options.loadingMessageVar %>'] = '{!LoadingMessage}';
                    window['<%= options.userInfo %>'] = {
                      display_name: '{! JSENCODE($User.FirstName) } {! JSENCODE($User.LastName) }',
                      first_name: '{! JSENCODE($User.FirstName) }',
                      last_name: '{! JSENCODE($User.LastName) }',
